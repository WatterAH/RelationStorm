...
interface Props {
  agregarBase: (base: { title: string, description: string, color: string }) => void
}
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button size="lg">
          <Plus className="h-5 w-5 mr-2" />Nueva Base de Datos
        </Button>
      </DialogTrigger>
              ...
              {colors.map((color, idx) => (
                <button
                  ...
                  className={`h-12 rounded-xl bg-gradient-to-r ${color} transition-all hover:scale-105 ${
                                form.color === color ? "ring-4 ring-offset-2 ring-gray-300" : ""
                              }`}/>
              ))}
            </div>
          </div>
